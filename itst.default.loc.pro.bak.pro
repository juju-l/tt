	...
	server {
		...
		location /index.htm {
			...
			return 302 http://dash.vipex.cc; #http://10.0.0.131:86/;
		}
		...
		location /auth {
			...
			proxy_pass http://10.0.0.131:10001/configApi;	include vhost/http/proxy.cnt.cfg;
		}
		
location @error401 {
		return 302 http://auth.vipex.cc/xXsys/auth; #http://10.0.0.131:10001/loginApi?user=sys&pswd=54321;
    }
